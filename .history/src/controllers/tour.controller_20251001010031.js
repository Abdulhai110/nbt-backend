...
exports.create = async (req, res) => {
  try {
    ...
    if (req.files?.coverImage) {
      ...
      data.coverImage = buildFilePath("tours/temp", filename); // temp until ID created
    }

    if (req.files?.images) {
      data.images = req.files.images.map(f => buildFilePath("tours/temp", f.filename));
    }
...
exports.update = async (req, res) => {
  try {
    ...
    if (req.files?.coverImage) {
      const filename = req.files.coverImage[0].filename;
      tour.coverImage = buildFilePath(`tours/${req.params.id}`, filename);
    }

    // Update gallery images
    if (req.files?.images) {
      tour.images = req.files.images.map(f =>
        buildFilePath(`tours/${req.params.id}`, f.filename)
      );
    }
