...
exports.getAllPublic = async (req, res, next) => {
  try {
    const result = await TourService.getPublicTours(req.query);
    res.status(200).json({ status: 'success', ...result.pagination, data: result.tours });
  } catch (err) { next(err); }
};
...
exports.update = async (req, res, next) => {
  try {
    const data = parseJsonFields({ ...req.body });
    const tour = await TourService.updateTour(req.params.id, data, req.files, req.user);
    res.status(200).json({ status: 'success', message: 'Tour updated successfully', data: tour });
  } catch (err) { next(err); }
};
